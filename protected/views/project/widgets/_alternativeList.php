<?php if(!empty($model->alternatives)) : ?>
    <div class="clearfix">
    <?php
    $width = 150;
    $spacing = 8;
    ?>
    <?php foreach($model->alternatives as $part): ?>
        <?php
        $images = $part->images; 
        ...
        <div class="polaroid" style="margin: 0px <?php echo $spacing; ?>px <?php echo $spacing*2; ?>px;">
            <div style="position:absolute;margin-left:-15px;margin-top:-18px;"><?php echo CHtml::link('<i class="icon-remove-sign"></i>','#',array('onClick'=>'removeAlternative('.$part->id.')')); ?></div>
