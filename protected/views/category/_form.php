...
<?php if (isset($_POST['create_root']) && $_POST['create_root']=='true' && $model->isNewRecord) : ?>              <h3 id="create_header">Create New Root Category </h3>
...
<?php   $formId='category-form';
 $ajaxUrl=($model->isNewRecord)?
              ( (isset($_POST['create_root']) && $_POST['create_root']='true')?CController::createUrl('category/createRoot'):CController::createUrl('category/create')):
               CController::createUrl('category/update');
$val_error_msg='Error.Category was not saved.';
$val_success_message=($model->isNewRecord)?
(isset($_POST['create_root']) && ($_POST['create_root']='true')?'Root Category was created successfuly.':'Category was created successfuly.'):
...
$success='function(data){
    ...
    if (response.success ==true)
    {
        ...
        $("#category-form").slideToggle(1500);'.
//        $updatesuccess.
    '}
         else {
                   $("#error-category")
...
<?php echo $form->errorSummary($model, '<div style="font-weight:bold">Please correct these errors:</div>', NULL, array('class' => 'errorsum notification errorshow png_bg')); ?><p class="note">Fields with <span class="required">*</span> are required.</p>
 ...
 <div class="row" >
  <?php echo $form->labelEx($model,'name'); ?>    <?php  echo $form->textField($model,'name',array('size'=>60,'maxlength'=>128,'value'=>$_POST['name'],'style'=>'width:75%;'));  ?>       <span  id="success-Category_name"  class="hid input-notification-success  success png_bg"></span>
    <div><small></small> </div>
